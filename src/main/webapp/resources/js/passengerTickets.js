$(document).ready(function () {
    ...
    function getAllTickets(data, typeOfTicket) {
        ...
        for(let i = 0; i < data.length;i++){
            markup += '<div class="item">' +
                ...
                '<div class="trainNumber_1">' + message[currentLocale].trainNumberSymbol +' ' + data[i].trainTicketDto.number + '</div></div></div>';
            markup += '<div class="wrapper_item">' +
                '<div class="wrapper_rout"><div class="route"><div class="label"><span>' + message[currentLocale].from + '</span></div>' +
                '<div class="departStation_1">' + data[i].trainTicketDto.departStation+ '</div></div><div class="arrow_right"></div>' +
                '<div class="route"><div class="label"><span>' + message[currentLocale].to + '</span></div>' +
                '<div class="arrivalStation_1">' + data[i].trainTicketDto.arrivalStation + '</div></div></div></div>';

            markup += '<div class="wrapper_item">' +
                ' <div class="date">' +
                '<div class="label"><span>' +  message[currentLocale].departureDate+ '</span></div>' +
    ...
    function buttonListener(){
        let upcoming = document.getElementById("upcoming");
        let completed = document.getElementById("completed");
        upcoming.addEventListener("click", getActualTickets, false);
        completed.addEventListener("click", getNotActualTickets, false);
    }

    function getNotActualTickets(){
        let currentLocale = window.locale;
        let passenger = {};
        passenger["lastName"] = document.getElementById('currentPassenger_lastName').value;
        passenger["name"] = document.getElementById('currentPassenger_name').value;
        ...
        $.ajax({
            ...
            success: function (data) {
                let  markup = '' +
                    '<div class="passenger_info">' +
                    '    <div>' +
                    '        <span>'+ message[currentLocale].passengerHeader+ ' </span>' +
                    '        <span>' + passenger["lastName"] + ' ' + passenger["name"] + '</span>' +
                    '    </div>' +
                    '</div>';
